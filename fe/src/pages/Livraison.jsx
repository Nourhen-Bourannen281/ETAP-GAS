...
// ==================== ASSIGNER TRANSPORTEUR (SIGNER LIVREUR) ====================
const assignTransporteur = async (livraisonId, transporteurId) => {
  ...
  try {
    setLoading(true);
    console.log('📌 Assignation - Livraison ID:', livraisonId);
    console.log('📌 Transporteur ID:', transporteurId);
    
    const response = await api.patch(`/livraisons/${livraisonId}/assign-transporteur`, { transporteurId });
    ...
    if (response.data.success) {
      ...
      // ✅ Mettre à jour la livraison dans l'état local IMMÉDIATEMENT
      setLivraisons(prevLivraisons => 
        prevLivraisons.map(liv => 
          liv._id === livraisonId 
            ? { ...liv, transporteur: response.data.livraison.transporteur }
            : liv
        )
      );
      
      // ✅ Recharger toutes les livraisons pour être sûr d'avoir les données à jour
      await fetchLivraisons();
      
    } else {
      addToast(response.data.message || 'Erreur lors de l\'assignation', 'error');
    }
